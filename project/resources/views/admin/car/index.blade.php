            ...
            <ul class="links">
               ...
               <li>
                 @if (request()->input('type') == 'all')
                   <a href="{{ route('admin.car.index').'?type='.request()->input('type') }}">All Cars</a>
                 @elseif (request()->input('type') == 'featured')
                   <a href="{{ route('admin.car.index').'?type='.request()->input('type') }}">Featured Cars</a>
                 @elseif (request()->input('type') == 'pending')
                   <a href="{{ route('admin.car.index').'?type='.request()->input('type') }}">Pending Cars</a>
                 @elseif (request()->input('type') == 'published')
                   <a href="{{ route('admin.car.index').'?type='.request()->input('type') }}">Published Cars</a>
                 @elseif (request()->input('type') == 'rejected')
                   <a href="{{ route('admin.car.index').'?type='.request()->input('type') }}">Rejected Cars</a>
                 @endif
               </li>
            </ul>
         </div>
...
<script type="text/javascript">
  var table = $('#geniustable').DataTable({
      ...
      processing: true,
      serverSide: true,
      ajax: '{{ route('admin.car.datatables', request()->input('type')) }}',
      columns: [{
              data: 'title',
              name: 'title'
          },
